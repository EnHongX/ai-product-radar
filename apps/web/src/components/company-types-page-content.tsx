...
export function CompanyTypesPageContent() {
  ...
  const typeT = t.companyTypes;

  const loadTypes = async () => {
    try {
      setLoading(true);
      const data = await fetchCompanyTypes();
      setTypes(data);
      setError(null);
    } catch (err) {
  ...
  const handleToggleStatus = async (typeItem: CompanyType) => {
    try {
      await updateCompanyType(typeItem.id, { enabled: !typeItem.enabled });
  ...
  const handleFormSubmit = async (data: CompanyTypeCreate) => {
    try {
      setIsSubmitting(true);
      setError(null);

      if (editingType) {
        const updateData: CompanyTypeUpdate = { name: data.name };
        await updateCompanyType(editingType.id, updateData);
      } else {
      ...
      setError(err instanceof Error ? err.message : "Failed to save company type");
    } finally {
